...
function read_data_2d(data::Array, ind::Int, dict::Dict, flipping::Vector)
...
function read_data(filename::String, dict::Dict, flipping::Vector)
    _data = read_nc(filename, dict["DATA NAME"]);

    # rotate the data if necessary
    if isnothing(dict["INDEX AXIS INDEX"])
        return read_data_2d(_data, 1, dict, flipping)
    else
        _eata = zeros(Float64, size(_data, dict["LONGITUDE AXIS INDEX"]), size(_data, dict["LONGITUDE AXIS INDEX"]), size(_data, dict["INDEX AXIS INDEX"]));
        for _ind in axes(_data, dict["INDEX AXIS INDEX"])
            _eata[:,:,_ind] .= read_data_2d(_data, _ind, dict, flipping);
        end;
...
function reprocess_data!(dict::Dict, file_name_function::Union{Function,Nothing})
    ...
    if _years == ""
        ...
        for _year in _years
            push!(_files, replace(_dict_file["FILE NAME PATTERN"], "XXXXXXXX" => file_name_function(_year)));
        end;
    end;
    ...
    if length(_dict_vars) == 1
        _reprocessed_data = read_data(_file, _dict_vars[1], _dict_file["LAT LON FLIPPING"]);
    else
        ...
        for _i_var in eachindex(_dict_vars)
            _reprocessed_data[:,:,_i_var] = read_data(_file, _dict_vars[_i_var], _dict_file["LAT LON FLIPPING"]);
        end;
    end;
...
local_data = reprocess_data!(local_json_dict, local_name_function);
